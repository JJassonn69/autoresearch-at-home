...
Each experiment runs on a single GPU. The training script runs for a **fixed FLOP budget** (≈8.9×10¹⁶ FLOPs, derived from an H100 running for 5 minutes at 30% MFU). Faster GPUs finish sooner; slower GPUs take longer — but every run does the same amount of compute, so results are directly comparable across hardware. You launch it simply as: `uv run train.py`.

**What you CAN do:**
- Modify `train.py` — this is the only file you edit. Everything is fair game: model architecture, optimizer, hyperparameters, training loop, batch size, model size, etc.

**What you CANNOT do:**
- Modify `prepare.py`. It is read-only. It contains the fixed evaluation, data loading, tokenizer, and training constants (FLOP budget, sequence length, etc).
- Install new packages or add dependencies. You can only use what's already in `pyproject.toml`.
- Modify the evaluation harness. The `evaluate_bpb` function in `prepare.py` is the ground truth metric.

**The goal is simple: get the lowest val_bpb.** Since the FLOP budget is fixed, every GPU does the same amount of compute — slower GPUs just take more wall-clock time. Everything is fair game: change the architecture, the optimizer, the hyperparameters, the batch size, the model size. The only constraint is that the code runs without crashing and finishes within the FLOP budget.
...
mfu_percent:      39.80
total_flops:      8.9055e+16
flop_budget:      8.9055e+16
...
Note that the script stops after exhausting the FLOP budget (~8.9×10¹⁶ FLOPs). Faster GPUs complete sooner; slower GPUs take longer. You can extract the key metric from the log file:
...
**Timeout**: Each experiment should complete within the FLOP budget (≈5 minutes on an H100, longer on slower GPUs). If a run exceeds 3× the expected wall-clock time for your GPU, kill it and treat it as a failure (discard and revert).
...
As an example use case, a user might leave you running while they sleep. On an H100 each experiment takes ~5 minutes (longer on slower GPUs), so you can run many experiments overnight. The user then wakes up to experimental results, all completed by you while they slept!
